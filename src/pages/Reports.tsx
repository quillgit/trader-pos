...
export default function Reports() {
    ...
    useEffect(() => {
        const loadDaily = async () => {
            ...
            // Calculate Totals
            // Inflow: Sales Paid Amount (Cash In)
            const totalIn = inflows.reduce((sum, t) => sum + (t.paid_amount || 0), 0);
            
            // Outflow: Purchases Paid Amount (Cash Out) + Expenses
            const totalPurchaseOut = outflows.reduce((sum, t) => sum + (t.paid_amount || 0), 0);
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                            ...
                            <div className="text-2xl font-bold text-blue-900">
                                {dailyData.session?.start_amount.toLocaleString() || '0'}
                            </div>
                        ...
                        <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                            ...
                            <div className="text-2xl font-bold text-green-900">
                                +{dailyData.totalIn.toLocaleString()}
                            </div>
                        ...
                        <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                            ...
                            <div className="text-2xl font-bold text-red-900">
                                -{dailyData.totalOut.toLocaleString()}
                            </div>
                    ...
                    <div className="bg-gray-800 text-white p-6 rounded-xl shadow-lg flex justify-between items-center">
                        ...
                        <div className="text-3xl font-bold">
                            {dailyData.balance.toLocaleString()}
                        </div>
                    </div>
                                    ...
                                    <tbody className="divide-y">
                                        {dailyData.inflows.map(t => (
                                            <tr key={t.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 font-medium">{t.partner_name || 'Cash Customer'}</td>
                                        ...
                                        {dailyData.inflows.length === 0 && (
                                            <tr><td colSpan={3} className="p-4 text-center text-gray-400">No sales today</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                    ...
                                    <tbody className="divide-y">
                                        {dailyData.outflows.map(t => (
                                            <tr key={t.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 font-medium">{t.partner_name || 'Supplier'} (Purchase)</td>
                                        ...
                                        {dailyData.expenses.map(e => (
                                            <tr key={e.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 font-medium">{e.description} ({e.category})</td>
                                                <td className="px-4 py-2 text-right text-red-600">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                            ...
                            <div className="text-2xl font-bold text-indigo-900">
                                {rangeData.totalSales.toLocaleString()}
                            </div>
                        ...
                        <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                            ...
                            <div className="text-2xl font-bold text-orange-900">
                                {rangeData.totalPurchases.toLocaleString()}
                            </div>
                        ...
                        <div className={`p-4 rounded-xl border ${rangeData.grossProfit >= 0 ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                            ...
                            <div className={`text-2xl font-bold ${rangeData.grossProfit >= 0 ? 'text-green-900' : 'text-red-900'}`}>
                                {rangeData.grossProfit > 0 ? '+' : ''}{rangeData.grossProfit.toLocaleString()}
                            </div>
                                ...
                                <tbody className="divide-y">
                                    {rangeData.sales.map(t => (
                                        <tr key={t.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3 font-medium">{t.partner_name || 'Cash Customer'}</td>
                                            <td className="px-4 py-3 text-gray-600 truncate max-w-xs">
                                                {t.items.map(i => `${i.product_name} (${i.quantity})`).join(', ')}
                                            </td>
                                            <td className="px-4 py-3 text-right font-medium">{t.total_amount.toLocaleString()}</td>
                                            <td className="px-4 py-3 text-right text-green-600">{t.paid_amount?.toLocaleString()}</td>
                                        </tr>
                                    ))}
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                            ...
                            <div className="text-2xl font-bold text-orange-900">
                                {debtData.totalReceivables.toLocaleString()}
                            </div>
                            <div className="text-xs text-orange-500 mt-1">
                        ...
                        <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                            ...
                            <div className="text-2xl font-bold text-red-900">
                                {debtData.totalPayables.toLocaleString()}
                            </div>
                            <div className="text-xs text-red-500 mt-1">
                                        ...
                                        {debtData.receivables.map(t => (
                                            <tr key={t.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 text-right font-medium text-orange-600">
                                                    {(t.total_amount - (t.paid_amount ?? 0)).toLocaleString()}
                                                </td>
                                            </tr>
                                        ))}
